 ...
 * 메인 모달
 * ============================================================ */
export function CardCollectionModal({ player, subscriptionState, onClose, onOpenPremium }: CardCollectionModalProps) {
  const { t } = useTranslation();
  ...
  const acquiredCount = CARD_COLLECTION.filter(
    (c) => getCardStatus(c, player.level, player.clearedLevel, premiumActive) === "acquired",
  ).length;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[200] flex flex-col bg-white animate-in slide-in-from-right duration-300">
      ...
      {/* ── 상단 헤더 */}
      <div className="flex items-center gap-3 px-5 pt-safe-top pt-4 pb-3 border-b border-foreground/8 flex-shrink-0">
        <button
          onClick={onClose}
          className="w-8 h-8 flex items-center justify-center rounded-full bg-foreground/8 hover:bg-foreground/14 active:scale-95 transition-all text-foreground/50"
        >
          ←
        </button>
        <div className="flex items-center gap-2 flex-1">
          <img src="/icons/icon-card.png" className="w-6 h-6 object-contain" alt="" draggable={false} />
          <h2 className="text-base font-bold text-foreground">{t("cardCollection.title")}</h2>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        ...
        <CardGroup
          ...
          badge={player.level >= 100 ? t("cardCollection.lv100BadgeUnlocked") : t("cardCollection.lv100BadgeLocked", { level: player.level })}
          badgeColor={player.level >= 100 ? "#10b981" : "#9ca3af"}
          cards={lv100Cards}
          playerLevel={player.level}
          clearedLevel={player.clearedLevel}
          premiumActive={premiumActive}
