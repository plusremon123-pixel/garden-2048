...
export function FrontScreen({
  ...
  return (
    <div className="flex flex-col items-center min-h-[100dvh] w-full bg-background px-4 py-4">

      {/* ── 광고 placeholder (상단) ─────────────────────── */}
      <AdBanner position="top" />

      {/* ── 레벨 바 ──────────────────────────────────────── */}
      <div className="w-full max-w-[420px] mt-3 mb-1">
        <LevelBar player={player} size="large" />
      </div>

      {/* ── 메인 콘텐츠 ──────────────────────────────────── */}
      <main className="flex flex-col items-center flex-1 w-full max-w-[420px] justify-center gap-7 py-4">
        ...
        <div className="text-center">
          ...
          {/* 코인 표시 */}
          {player.coins > 0 && (
            <p className="mt-1 text-xs text-amber-500 font-bold">
              🪙 {player.coins.toLocaleString()} 코인
            </p>
          )}
        </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {THEME_ORDER.map((id) => {
              ...
              return (
                ...
                >
                  {isSelected && (
                    <span className="absolute top-2 right-2 w-5 h-5 bg-primary rounded-full flex items-center justify-center text-white text-xs">
                      ✓
                    </span>
                  )}
